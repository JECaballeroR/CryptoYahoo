...
st.write(
    "Este aplicación extrae la información de Yahoo Finance **en tiempo real** para el Bitcoin y el Ethereum en dólares y la presenta"
...
st.write(
    "Esta aplicación fue desarrollada por Jorge Esteban Caballero R para la entrega 'Dashboard 2: Una herramienta propia con código' "
...
class DatosTiempoReal():
    ...
    def data_proc(self, ticker, price, timestamp) -> None:
        if not self.data[ticker] or self.data[ticker][-1][
            ...
            df = pd.DataFrame(columns=['COIN', 'Tiempo', 'Precio (USD)'],
                              data=[[ticker, datetime.datetime.now(), price]])
